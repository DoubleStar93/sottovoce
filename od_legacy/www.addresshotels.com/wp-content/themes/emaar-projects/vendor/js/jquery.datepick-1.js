/*! http://keith-wood.name/datepick.html
	...
	$(selector).datepick({minDate: 0, maxDate: '+1m +1w'}) */
		$.JQPlugin.createPlugin({
			...
			defaultRenderer: {
				...
				todayClass: 'datepick-today today',
			...
			commands: {
				prev: {text: 'prevText', status: 'prevStatus', // Previous month
					...
					enabled: function(inst) {
						...
						//the code is added to show the previous icon when dates are seleted - JAYANT
						const url = window.location.href;
						// if (url.includes("offers")) {
						// 	var minDate = inst.curMinDate();
						// } else {
							var minDate = new Date(moment().utcOffset(offset).format("YYYY/MM/DD"))
						//}
						//var minDate = inst.curMinDate(); -> this is the old code commented for showing previous Icon when dates are selected -  JAYANT
	...
	$.datepick.newDate(2014, 12, 25) */
			...
			_checkOffset: function(inst) {
				...
				if(JSON.stringify(offset)==undefined)
				{
					offset = {"top":591.5,"left":62};
				}
			...
			_generateMonth: function(elem, inst, year, month, renderer, first) {
				...
				for (var week = 0; week < numWeeks; week++) {
					...
					for (var day = 0; day < 7; day++) {
						...
						//this code is added so that the previous date do not get dis selected when user selects a date and the vairable is set according to OFFERS page - JAYANT
						const url = window.location.href;
						// if (url.includes("offers")) {
						// 	minDate = (inst.pickingRange ? inst.selectedDates[0] : inst.get('minDate'));
						// } else {
						// 	minDate = inst.get('minDate');
						// }
						minDate = inst.get('minDate');
						//minDate = (inst.pickingRange ? inst.selectedDates[0] : inst.get('minDate')); ->old code commeted - JAYANT
